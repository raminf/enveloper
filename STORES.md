...
This guide explains how to create new store plugins for Enveloper and add them to the system.

## Overview

Enveloper uses a plugin system based on Python entry points to discover and load store implementations. The recommended workflow is to **fork the enveloper repository**, add your store under `src/enveloper/stores/`, register the entry point, and submit a PR (or maintain your fork).
...
For example, with the AWS store (separator `/`) the key for `API_KEY` might be:

```
/envr/payments/myapp/1.0.0/API_KEY
```

The leading `/` is added by the AWS store itself -- each store can prepend its own separator if the backend requires it.
...
Otherwise the default `get_service_rows()` returns a single row from the three class attributes.
...
```bash
poetry install
poetry run enveloper stores          # should show my_backend
poetry run enveloper service         # should list my_backend with description and doc link
poetry run enveloper --service my_backend list
```
...
- Re-run `poetry install` after editing `pyproject.toml`.
